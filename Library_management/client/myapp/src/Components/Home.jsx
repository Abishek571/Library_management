...
function Home() {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get('https://library-management-p24k.onrender.com/allbooks');
  ...
  const handleCountSubmit = async () => {
    try {
      const response = await axios.get(
        `https://library-management-p24k.onrender.com/bookcount/${author}/${name}/${subject}/${date}`
      );
  ...
  const handleSearch = async () => {
    try {
      const response = await axios.get(
        `https://library-management-p24k.onrender.com/searchbooks/${searchTerm}`
      );
      setBooks(response.data.books);
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`https://library-management-p24k.onrender.com/deletebook/${id}`);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post('https://library-management-p24k.onrender.com/insertbook', formData);
